...
// https://github.com/giggio/globdir
namespace GlobDir
{
    public static class Glob
    {
        ...
        public static bool HasGlobPattern(string s)
        {
            bool res = false;

            string glob = "*,?,\\,[,],:"; // I want to add "." but for my case, this is not necessary
            foreach (string a in glob.Split(','))
            {
                res = s.Contains(a);
                if (res) break;
            }
        ...
        private class PlatformAdaptationLayer
        {
            ...
            {
                // this is causing some UnauthorizedAccessException problem
                ...
                try
                {
                    files = Directory.EnumerateFileSystemEntries(path, searchPattern, SearchOption.TopDirectoryOnly);
                }
